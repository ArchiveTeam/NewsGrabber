...
def grab_list(listname):
	...
	os.system('~/.local/bin/grab-site --input-file ./old_lists/' + listname + ' --level=0 --no-sitemaps --concurrency=5 --1 --warc-max-size=524288000 --wpull-args="' + videostring + '--no-check-certificate --timeout=300" > /dev/null 2>&1')
...
def main():
	...
	threading.Thread(target = grab_new_lists).start()
